...
- [ ] 需具備斷線偵測與自動重連機制。
- [ ] 實作可測試的事件流緩衝、降頻、共享訂閱與重連機制，在高頻更新下頁面不卡頓；RxJS 為建議方案而非唯一方案。
...
- [ ] 核心邏輯需具清楚命名、模組邊界與必要的技術說明；僅對複雜規則補充註解。
- [ ] 程式碼架構需遵循 Clean Code 原則，元件職責明確，並優先採領域導向的檔案分類。
- [ ] 非跨領域共用的元件、hooks、型別與工具函式，應收斂在對應領域資料夾內管理。
...
- [ ] 針對數據處理邏輯，如技術指標計算與市場事件流編排，撰寫 unit tests。
- [ ] 核心邏輯測試覆蓋率需達到 80% 以上。
...
- [ ] 所有使用者輸入欄位需實作 trim、長度限制、格式驗證與安全編碼，不得直接拼接未驗證字串。
...
- [ ] Lighthouse Performance 分數需達到 90 分以上，並以瀏覽器效能 trace 驗證高頻更新情境。
...
- 第一階段可先以公開資料與前端持久化完成核心功能；正式交付版需保留可切換至 BFF 的架構邊界。
